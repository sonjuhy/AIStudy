...
class MobileNetV4ConvSmallBackbone(nn.Module):
    """
    returns: tuple(P3, P4, P5)
    - P3: stride 8  (1/8 해상도)
    - P4: stride 16 (1/16)
    - P5: stride 32 (1/32)
    out_channels: 각 피처맵의 채널 수 리스트를 기억해두면 neck/head에서 씁니다.
    """

    def __init__(self, num_classes: int = 80):
        super().__init__()

        base_model = MobileNetV4ConvSmall(num_classes=num_classes)

        self.stage1 = base_model.model[:2]  # 0,1 (stride 4)
        self.stage2 = base_model.model[2:3]  # 2   (stride 8, C=64)   -> P3
        self.stage3 = base_model.model[3:9]  # 3~8 (stride 16, C=96) -> P4
        self.stage4 = base_model.model[9:15]  # 9~14(stride 32, C=128)-> P5

        self.out_channels = [64, 96, 128]

    def forward(self, x):
        x = self.stage1(x)  # [B, 48, H/4, W/4]
        p3 = self.stage2(x)  # [B, 64, H/8, W/8]   <-- 96에서 64로 수정
        p4 = self.stage3(p3)  # [B, 96, H/16, W/16] <-- 192에서 96으로 수정
        p5 = self.stage4(p4)  # [B, 128, H/32, W/32] <-- 512에서 128로 수정
        return p3, p4, p5


class MobileNetV4ConvMediumBackbone(nn.Module):
    """
    returns: tuple(P3, P4, P5)
    - P3: stride 8  (1/8 해상도)
    - P4: stride 16 (1/16)
    ...
    def __init__(self, num_classes: int = 80):
        ...
        base_model = MobileNetV4ConvMedium(num_classes=num_classes)

        # 0: conv(3->32, s=2)          -> /2
        # 1: FusedIB(32->48, s=2)      -> /4, C=48
        self.stage1 = base_model.model[:2]  # 0,1  (stride 4,  C=48)

        # 2: UIB(48->80, s=2)          -> /8, C=80
        # 3: UIB(80->80, s=1)          -> /8, C=80
        self.stage2 = base_model.model[2:4]  # 2~3 (stride 8,  C=80)   -> P3

        # 4: UIB(80->160, s=2)         -> /16, C=160
        # 5~11: UIB(..., s=1)          -> /16, C=160
        self.stage3 = base_model.model[4:12]  # 4~11(stride 16, C=160)  -> P4

        # 12: UIB(160->256, s=2)       -> /32, C=256
        # 13~22: UIB(..., s=1)         -> /32, C=256
        # 23: conv(256->960, s=1)      -> /32, C=960 (백본 P5에는 보통 포함 안 함)
        self.stage4 = base_model.model[12:23]  # 12~22(stride 32, C=256) -> P5

        # 출력 채널 수 기록 (추후 neck/head에서 사용 가능)
        self.out_channels = [80, 160, 256]

    def forward(self, x):
        x = self.stage1(x)  # [B, 48, H/4, W/4]
        p3 = self.stage2(x)  # [B, 80, H/8, W/8]   <-- 96에서 80으로 수정
        p4 = self.stage3(p3)  # [B, 160, H/16, W/16] <-- 192에서 160으로 수정
        p5 = self.stage4(p4)  # [B, 256, H/32, W/32] <-- 512에서 256으로 수정
        return p3, p4, p5


class MobileNetV4ConvLargeBackbone(nn.Module):
    """
    returns: tuple(P3, P4, P5)
    - P3: stride 8  (1/8 해상도)
    - P4: stride 16 (1/16)
    ...
    def __init__(self, num_classes: int = 80):
        super().__init__()
        base_model = MobileNetV4ConvLarge(num_classes=num_classes)

        # 0~1: stride 2, 4 (C=48)
        self.stage1 = base_model.model[:2]

        # 2~3: stride 8 (C=96) -> P3
        # 이전 코드 [2:3]에서 [2:4]로 수정해야 정상적인 P3 블록이 구성됨
        self.stage2 = base_model.model[2:4]

        # 4~8: stride 16 (C=192) -> P4
        # 이전 코드 [3:5]는 완전히 잘못된 영역을 가리킴
        self.stage3 = base_model.model[4:9]

        # 9~14: stride 32 (C=512) -> P5 (마지막 1x1 conv 제외)
        self.stage4 = base_model.model[9:15]

        self.out_channels = [96, 192, 512]
